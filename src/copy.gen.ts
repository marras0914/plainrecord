...
export const EN: Record<CopyKey, string> = {
  ...
  "author.donation": "<b>I donate to the Democratic Party.</b> You should know that before you read anything else here, and it is the reason this page is built the way it is rather than a footnote. I have a side, so the rule that picks the comparison members runs identically on both caucuses. A third of the questions are reserved for votes where the two parties <em>agreed</em>, the selection rule is named and published, and every vote, count and score sits in one file you can download and check. None of that asks you to trust me, and that is the whole point of it.",
  "author.ai": "<b>I used AI to build this.</b> The code and most of the writing were drafted with Claude and edited by me. None of it touches the record. Every vote, count and date comes from the Texas House journals and LegiScan, the arithmetic is checked on every build, and the whole file is downloadable, so you can check any figure here against the source instead of taking my word for it.",
  ...
  "method.denominator.body": "The percentage in the tiles above counts all {items} questions. The <code>crossCuttingShare</code> field in the data file is measured over the rule-selected votes only, so it is a slightly different number. That is deliberate and it exists to check the rule. The seven headline bills were picked by hand rather than by the rule, six of them split the parties sharply, so counting them would make the rule look more one sided than it is. Both numbers are honest and neither is the other one rounded.",
  ...
  "privacy.body": "The site counts visits, how many people came, which link sent them, and how many reach each step of the quiz, using Vercel’s analytics, which sets no cookies and cannot follow you to another site. <b>Your answers are not part of that.</b> They never leave your browser while you answer. Reaching the end is counted; what you answered is not, and the two are never connected. If you then tap <b>add my result</b> on the result screen, one request goes out, and it carries where you landed, three numbers about your answers, and which way you answered each vote. No name, no address, or anything that identifies you, and only totals are kept at the other end. There is no record of your particular submission to look up. If instead you tap <b>challenge someone</b>, you get a link holding your seven answers in the part of the address after the # symbol, which browsers never transmit, so nothing about it reaches this site at all. If you ask the page to find your district, it downloads the district map to your device. That is a request for a file and carries nothing about you. Tap none of them and nothing but the visit count is ever sent. This is checked on every build, the test answers five questions and fails if a single byte goes out beyond that count, then taps each button and fails if what goes out carries anything more.",
  ...
  "rep.zipSpans": "ZIP {zip} is split across {n} House districts. Which one you are in comes down to your street, so pick yours below, or get an exact answer from {link}. The percentages are shares of land, not of people, so the biggest is not the likeliest.",
  "rep.zipShare": "{pct}% of this ZIP's land",
  "rep.zipShareSmall": "under 1% of this ZIP's land",
...
export const ES: Record<CopyKey, string> = {
  ...
  "author.donation": "<b>Yo dono al Partido Demócrata.</b> Debería saberlo antes de leer cualquier otra cosa aquí, y es la razón por la que esta página está construida como está, no una nota al pie. Tengo un lado, por eso la regla que escoge a los miembros de comparación funciona igual en las dos bancadas. Un tercio de las preguntas se reserva para votos en los que los dos partidos <em>estuvieron de acuerdo</em>, la regla de selección tiene nombre y está publicada, y cada voto, conteo y calificación está en un archivo que usted puede descargar y verificar. Nada de eso le pide que confíe en mí, y ese es exactamente el punto.",
  "author.ai": "<b>Usé inteligencia artificial para hacer esto.</b> El código y la mayor parte de los textos los redactó Claude y yo los edité. Nada de eso toca el registro. Cada voto, conteo y fecha viene de los diarios de la Cámara de Texas y de LegiScan, la aritmética se comprueba en cada compilación, y el archivo completo se puede descargar, así que usted puede verificar cualquier cifra de esta página contra la fuente en lugar de creerme a mí.",
  ...
  "method.denominator.body": "El porcentaje de los recuadros de arriba cuenta las {items} preguntas. El campo <code>crossCuttingShare</code> del archivo de datos se mide solo sobre los votos escogidos por la regla, así que es una cifra ligeramente distinta. Eso es a propósito y existe para comprobar la regla. Los siete proyectos principales los escogimos a mano y no por regla; seis de ellos dividieron a los partidos con claridad, así que contarlos haría parecer la regla más sesgada de lo que es. Las dos cifras son honestas y ninguna es la otra redondeada.",
  ...
  "privacy.body": "El sitio cuenta visitas, cuántas personas vinieron, qué enlace las envió y cuántas llegan a cada paso del cuestionario, con la analítica de Vercel, que no usa cookies y no puede seguir su rastro en otro sitio. <b>Sus respuestas no forman parte de eso.</b> Nunca salen de su navegador mientras responde. Se cuenta que usted llegó al final; no se cuenta lo que respondió, y las dos cosas nunca se conectan. Si luego toca <b>añadir mi resultado</b> en la pantalla de resultados, sale una sola petición que lleva dónde quedó, tres números sobre sus respuestas y en qué sentido respondió cada voto. Sin nombre, sin dirección, ni nada que revele su identidad, y al otro lado solo se guardan totales. No queda registro de su envío concreto que se pueda consultar. Si en cambio toca <b>desafiar a alguien</b>, obtiene un enlace que lleva sus siete respuestas en la parte de la dirección después del símbolo #, que los navegadores nunca transmiten, así que nada de eso llega a este sitio. Si le pide a la página que encuentre su distrito, esta descarga el mapa de distritos a su dispositivo. Eso es una petición de un archivo y no lleva nada sobre usted. Si no toca ninguno, nunca se envía nada más que el recuento de visitas. Esto se comprueba en cada compilación: la prueba responde cinco preguntas y falla si sale un solo byte más que ese recuento, y después toca cada botón y falla si lo que sale lleva algo más.",
  ...
  "rep.zipSpans": "El código postal {zip} está repartido entre {n} distritos de la Cámara. Cuál le corresponde depende de su calle, así que elija el suyo abajo, o consulte {link} para una respuesta exacta. Los porcentajes son proporciones de superficie, no de habitantes, así que el mayor no es el más probable.",
  "rep.zipShare": "{pct}% de la superficie de este código postal",
  "rep.zipShareSmall": "menos del 1% de la superficie de este código postal",
